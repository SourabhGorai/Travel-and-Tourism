...
public class deleteDetails extends JFrame implements ActionListener {
    ...
    deleteDetails(String usrn) {
        ...
        deleteButton.setBounds(330, 350, 120, 25);
        deleteButton.setFocusable(false);
        deleteButton.setFont(new Font("Osward", Font.BOLD, 15));
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == deleteButton) {
            int response = JOptionPane.showConfirmDialog(null, "Do you want to delete all the details?");
            if (response == JOptionPane.YES_OPTION) {
                String query2 = "DELETE FROM add_cus WHERE username = '" + usrn + "';";
                try {
                    Conn c = new Conn();
                    c.s.executeUpdate(query2);
                    JOptionPane.showMessageDialog(null, "Data of " + usrn + " Deleted Successfully");
                    // Optionally close the window or perform other actions after deletion
